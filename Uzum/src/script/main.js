...
document.addEventListener("click", (e) => {
  if (!topMenuEl.contains(e.target) && !Array.from(kategorias).some(btn => btn.contains(e.target))) {
...
document.addEventListener('DOMContentLoaded', () => {
  const btn = document.querySelector('.katalog-btn');
  
  if (!btn) {
    console.warn('Элемент .katalog-btn не найден');
    return;
  }

  const catalog = document.createElement('div');
  catalog.id = 'catalog-menu';
  catalog.style.display = 'none';
  catalog.style.position = 'absolute';
  catalog.style.background = '#fff';
  catalog.style.border = '1px solid #ccc';
  catalog.style.padding = '10px';
  ...
  btn.addEventListener('click', (e) => {
    ...
    const isOpen = catalog.style.display === 'block';
    catalog.style.display = isOpen ? 'none' : 'block';

    const rect = btn.getBoundingClientRect();
    catalog.style.top = (rect.bottom + window.scrollY) + 'px';
    catalog.style.left = (rect.left + window.scrollX) + 'px';
  });

  document.addEventListener('click', (e) => {
    if (!catalog.contains(e.target) && !btn.contains(e.target)) {
      catalog.style.display = 'none';
    }
  });
});